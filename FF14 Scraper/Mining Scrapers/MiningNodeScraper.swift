...
//        try scrapeGatheringNodes(gamerEscapeURL: gamerEscapeWikiURL)
    ...
    private func scrapeGatheringNodes(gamerEscapeURL: URL, testItem: String = "/wiki/Wind_Cluster") throws {
        ...
        for tr in infoPanelBottomTBody.children() {
            guard let td1 = tr.children().first(),
                ...
                print("unable to get items in lines 151 - 154")
                continue
            }
            itemLvl = td2Text
        }
        ...
        for gatheringSource in ["Quarrying", "Mining"] {
            ...
            for tr in gatheringSourceTBody.children() {
                ...
                if itemTimes.isEmpty {
                    let locationInfo = NodeLocationInfo(time: time, location: itemLocation, stars: itemStars, x: x, y: y)
